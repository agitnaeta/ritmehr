...
|---|---|---|---|---|
| SEC-1 | 🔴 Kritis | **Tidak ada throttle di semua endpoint auth.** `RateLimiter::for` hanya untuk `'api'` (`RouteServiceProvider.php:27`); login kandidat/portal & Backpack login tak dibatasi. Brute-force/credential-stuffing terbuka. | `routes/web.php:41-43,97`, `config/backpack/base.php` (hanya throttle recovery/verify, bukan login) | Tambah `->middleware('throttle:5,1')` pada `career/masuk`, `career/daftar`, `/my/password`; aktifkan throttle login Backpack. Definisikan limiter `login` per-email+IP. |
